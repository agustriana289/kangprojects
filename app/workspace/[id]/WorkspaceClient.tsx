...
export default function WorkspaceClient({ order, user, isAdmin, clientName, adminName }: { 
  ...
  const handleAutoPortfolioUpload = async (e: any) => {
    ...
    try {
      ...
      const title = getProjectName().split(' — ')[1] || getProjectName();
      const category = order.store_services?.category || order.store_products?.category || "Logo Design";
      const description = `Portfolio for project ${getProjectName()}. Crafted for ${clientName}.`;
        ...
        if (!ws && !fetchError) {
          ...
          if (insertError) {
            if (insertError.code === '23505') {
              // Jika terjadi race condition (sudah terbuat di request lain), fetch ulang
              const { data: fetchRetry } = await supabase
                .from("store_workspaces")
                .select("id, order_id, user_id, status")
                .eq("order_id", order.id)
                .single();
             ...
             <div className="flex items-center gap-3">
                ...
                <div>
                   ...
                   <p className="text-xs font-medium text-slate-500">
                      ...
                      {(!isAdmin && counterpartRole === "Administrator") ? "Admin" 
                       : (isAdmin && order.user_id ? "Registered Client" : "Offline Client")}
                   </p>
                </div>
             </div>
                    ...
                    <>
                      ...
                      <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-slate-200 shadow-lg rounded-xl p-1.5 z-50">
                        ...
                        <button type="button"
                          ...
                          className={`w-full text-left px-3 py-2 rounded-lg flex items-center justify-between transition-all ${sortOrder === 'asc' ? 'bg-indigo-50 text-primary' : 'hover:bg-slate-50 text-slate-700'}`}
                        >
                          <div className="flex items-center gap-2">
                        ...
                        <button type="button"
                          ...
                          className={`w-full text-left px-3 py-2 rounded-lg flex items-center justify-between transition-all ${sortOrder === 'desc' ? 'bg-indigo-50 text-primary' : 'hover:bg-slate-50 text-slate-700'}`}
                        >
                          <div className="flex items-center gap-2">
              ...
              <>
                  ...
                  filteredMessages.map((msg, i) => {
                    ...
                    return (
                      <div key={i} className={`flex gap-3 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                        <div className="w-7 h-7 rounded-lg bg-indigo-50 flex items-center justify-center text-primary font-bold text-xs shrink-0 self-end">
                ...
                {sortOrder === "desc" && visibleCount < messages.length && !searchQuery && (
                  <div className="flex justify-center pt-4">
                    <button type="button" onClick={() => setVisibleCount(prev => prev + 20)} className="text-xs font-bold uppercase tracking-wider text-primary bg-indigo-50 px-4 py-2 rounded-lg hover:bg-indigo-100 transition-all">
                       Load more messages
                    </button>
                  </div>
                )}
              ...
              <div className="p-3 bg-white border border-slate-200 rounded-xl">
                 ...
                 {isAdmin ? (
                   <div className="relative">
                     ...
                     {updatingStatus && <Loader2 className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 animate-spin text-primary" />}
                   </div>
                 ) : (
                   <span className={`text-[10px] font-bold px-2 py-1 rounded border uppercase tracking-wider inline-block ${
                      currentStatus === 'processing' ? 'bg-indigo-50 text-primary border-indigo-200' :
              ...
              {isAdmin && isCompleted && (
                <div className="pt-4 mt-2 border-t border-slate-100">
                  ...
                  >
                    ...
                    <div>
                      <h4 className="text-xs font-bold uppercase tracking-wider">{uploadingPortfolio ? "Publishing..." : "Publish Portfolio"}</h4>
                      <p className="text-[10px] font-medium text-indigo-100 mt-0.5">{uploadingPortfolio ? "Uploading files..." : "Showcase this project"}</p>
                    </div>
                  </button>
                  <input 
                    type="file" 
